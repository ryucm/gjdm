					...
					<table summary="">
						...
						<thead>
							<tr>
								...
								<th scope="col">시도 코드</th>
								<th scope="col">시도</th>
								<th scope="col">군구 코드</th>
								<th scope="col">군구</th>
							...
							<c:forEach items="${dimDistrictList}" var="vo">
								...
								<tr style="display: none" name="${vo.districtId}" class="editable">
									...
									<td><input type="text" class="inputArea" style="width: 90%;"
											name="distLvl1Cd${vo.districtId}" value="${vo.distLvl1Cd}" required></td>
									<td><input type="text" class="inputArea" style="width: 90%;"
											name="distLvl1${vo.districtId}" value="${vo.distLvl1}" required></td>
									<td><input type="text" class="inputArea" style="width: 90%;"
											name="distLvl2Cd${vo.districtId}" value="${vo.distLvl2Cd}" required></td>
									<td><input type="text" class="inputArea" style="width: 90%;"
		...
		<div class="layBox">
			...
			<div class="layerCon">
				<div class="pop_tableType1">
					<form name="insert" method="post" id="insertForm">
						<table>
							<tr>
								<td>
									<p>국가</p>
							...
							<tr>
								<td>
									<p>시도 코드</p>
									<input type="text" placeholder="시도 코드" class="inputArea" name="distLvl1Cd" required
										style="width:100%;">
								</td>
							</tr>
							<tr>
							...
							<tr>
								<td>
									<p>군구 코드</p>
									<input type="text" placeholder="군구 코드" class="inputArea" name="distLvl2Cd" required
										style="width:100%;">
								</td>
							</tr>
							<tr>
...
<script>
	...
	function updateDistrict(districtId) {
		if (confirm("지역 ID '" + districtId + "' 를 수정하시겠습니까?")) {
			...
			var distLvl1Cd = $('input[name="distLvl1Cd' + districtId + '"]').val();
			var distLvl1 = $('input[name="distLvl1' + districtId + '"]').val();
			var distLvl2Cd = $('input[name="distLvl2Cd' + districtId + '"]').val();
	...
	function insertDistrict() {
		const regex = /^[0-9]*$/;
		if (!regex.test(document.insert.distLvl1Cd.value)) {
			alert("코드는 숫자만 입력 가능합니다.")
			return false
		}
		if (!regex.test(document.insert.distLvl2Cd.value)) {
			alert("코드는 숫자만 입력 가능합니다.")
			return false
		}
